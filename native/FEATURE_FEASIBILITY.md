...
| Frame loop is wall-clock paced at 16.667 ms with single-player catch-up (up to 3 retraces per wait, `MELEE_VI_CATCHUP`; logic stays near 60 Hz on a slow renderer, displayed frames drop). Off in deterministic/online mode, which needs one render per logic frame. | `native/vi_pacing.h`, `native/vi_runtime.cpp`, `native/ONLINE_PLAY.md` | Same-binary input replay is deterministic per logic frame; cross-platform (Dolphin) parity is not. HUD/item code reads render-computed state, so full-speed online would need that decoupled. |
