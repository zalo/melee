...
#ifdef MELEE_NATIVE
static gmClassicRuntimeData native_classic_runtime;
#define gmClassicIntroDataBuffer native_classic_runtime.intro
#define gm_804908A0 native_classic_runtime.state.bytes
#define CLASSIC_MATCHUPS(scene) gmClassic_803DDEC8
#else
gmClassicIntroData gmClassicIntroDataBuffer;
#define CLASSIC_MATCHUPS(scene) ((scene)->matchups)
#endif
...
static gm_803DDEC8Struct* gmClassic_801B2D54(gm_803DDEC8Struct* arg0)
{
    ...
    for (ptr = arg0; ptr->x0 != 0xD; ptr++) {
        if (ptr->x1 & 8) {
            gmClassicMatchup* result = gmClassic_801B2BA4(
                CLASSIC_MATCHUPS(scene_data).x2B0, o->state.order.x60, arg0);
            if (result != NULL) {
                ptr->xC = result;
            } else {
                for (;;) {
    ...
    for (ptr = arg0; ptr->x0 != 0xD; ptr++) {
        ...
        if ((flags & 2) && !(flags & 0x20)) {
            gmClassicMatchup* result = gmClassic_801B2BA4(
                CLASSIC_MATCHUPS(scene_data).x26C, o->state.order.x54, arg0);
            if (result != NULL) {
                ptr->xC = result;
            } else {
                for (;;) {
    ...
    for (ptr = arg0; ptr->x0 != 0xD; ptr++) {
        ...
        if ((flags & 0x10) && !(flags & 0x20)) {
            gmClassicMatchup* result = gmClassic_801B2BA4(
                CLASSIC_MATCHUPS(scene_data).x1B8, o->state.order.x34, arg0);
            if (result != NULL) {
                ptr->xC = result;
            } else {
                for (;;) {
    ...
    for (ptr = arg0; ptr->x0 != 0xD; ptr++) {
        ...
        if (flags == 0 || flags == 4) {
            gmClassicMatchup* result = gmClassic_801B2BA4(
                CLASSIC_MATCHUPS(scene_data).x0CC, o->state.order.x0C, arg0);
            if (result != NULL) {
                ptr->xC = result;
            } else {
                for (;;) {
    ...
    for (ptr = arg0; ptr->x0 != 0xD; ptr++) {
        if (ptr->x1 & 0x20) {
            ptr->xC = CLASSIC_MATCHUPS(scene_data).x0C0;
            return ptr;
        }
    }
    return ptr;
...
void gm_Mode_Classic_OnLoad(void)
{
    ...
    for (entry = CLASSIC_MATCHUPS(scene_data).x00; entry->x0 != 0x0D; entry++) {
        entry->xC = NULL;
    }

    gmClassic_InitMatchupOrder(CLASSIC_MATCHUPS(scene_data).x2B0,
                               (gmClassicOrderIndex*) &gm_804908A0[0x60],
                               (gmClassicOrderIndex*) o, 0x80);
    gmClassic_InitMatchupOrder(CLASSIC_MATCHUPS(scene_data).x26C,
                               (gmClassicOrderIndex*) &gm_804908A0[0x54],
                               (gmClassicOrderIndex*) o, 0x74);
    gmClassic_InitMatchupOrder(CLASSIC_MATCHUPS(scene_data).x1B8,
                               (gmClassicOrderIndex*) &gm_804908A0[0x34],
                               (gmClassicOrderIndex*) o, 0x54);
    gmClassic_InitMatchupOrder(CLASSIC_MATCHUPS(scene_data).x0CC,
