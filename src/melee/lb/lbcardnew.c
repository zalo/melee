...
#ifdef MELEE_NATIVE
    /* memcpy reads CARD_FILENAME_MAX bytes even from a shorter string. */
    strncpy(task->filename, filename, new_var);
#else
...
#ifdef MELEE_NATIVE
    /* memcpy reads CARD_FILENAME_MAX bytes even from a shorter string. */
    strncpy(setupTask(LbCardNewTask_Create, 0x10)->filename, filename, size);
#else
