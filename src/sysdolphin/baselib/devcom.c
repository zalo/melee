...
#ifdef MELEE_NATIVE
                // Preserve completion unless the caller explicitly replaces it.
                // Synth cancellation waits for this callback to retire the bank.
                if (flags & 1) dc->callback = cb;
                if (flags & 2) dc->args = args;
#else
                dc->callback = cb;
                dc->args = args;
#endif
