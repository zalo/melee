...
#ifdef MELEE_NATIVE
/* Retail places these three symbols back to back and the ring runner and
 * request builders address all of them through a CardContext cast on
 * hsd_804D1138. Separate host globals have no guaranteed adjacency, so the
 * native build keeps them in one CardContext. */
static CardContext hsd_native_card_context;
#define active_requests (hsd_native_card_context.active)
#define commands (hsd_native_card_context.cmds)
#define card_requests (hsd_native_card_context.requests)
#else
...
#ifdef MELEE_NATIVE
                    u8* src = state->sector_buf + hdr_offset;
#else
    ...
    case CARD_CMD_SCAN_BLOCK:
        ...
        if (phys == 0) {
            ...
            if (state->block_ids[phys] != 0) {
                ...
                {
#ifdef MELEE_NATIVE
                    u8* src13 = state->sector_buf + hdr_offset13;
#else
...
void fn_803AC3F8(CardState* state, u8* data, s32 file_idx)
{
    ...
    for (i = 0; i < 3 && i < 9; i++) {
        s32 idx = start + i;
#ifdef MELEE_NATIVE
        /* fn_803B1338 pads spare blocks with file_idx 9, so the console reads
         * file_sizes[9], which aliases file_data[0], and stores it as a bogus
         * tenth descriptor. Emit an empty entry instead. */
        if (idx >= HSD_CARD_MAX_FILES) {
            data[0] = 0;
            data[1] = 0;
            data[2] = 0;
            data[3] = 0;
...
#ifdef MELEE_NATIVE
        if (file_idx >= HSD_CARD_MAX_FILES) {
                                               ...
                                               CardCmdBuf* buf)
{
     ...
     * the sub-file size, so copy only what the buffer holds; the WRITE_BLOCK
     * handler zero-fills the rest of the sector. */
    if (size > state->file_sizes[file_idx] - offset) {
        size = state->file_sizes[file_idx] - offset;
        if (size < 0) {
            size = 0;
        }
    }
...
#ifdef MELEE_NATIVE
                            s32 payload = state->sector_size - 0x20;
                            if (payload > state->file_sizes[index] - offset) {
                                payload = state->file_sizes[index] - offset;
                                if (payload < 0) {
                                    payload = 0;
                                }
                            }
